<div class="navbar-fixed-top container" style="background-color:white;z-index:18">
		
	<div class="nav-lay nav-hid colorido-gl">
		 
                     <span class="nav-close ion-close"></span> 
                   	<span class="nav-item"><a href="index">Home</a></span>
					<span class="nav-item"><a href="blog">Blogs</a></span>
					<span  class="nav-item"><a href="infrastructure">Infrastructure</a></span>
					<span class="nav-item"><a href="faculty">Faculty</a></span>
					<span class="nav-item"><a href="contact_us">Contact us</a></span>
					<span class="nav-item"><a href="areas">Areas</a></span>
					<span  class="nav-item"><a href="areas#title">Interfaith Understanding</a></span>
					<span class="nav-item"><a href="areas#title1">Scriptural Studies</a></span>
					<span class="nav-item"><a href="areas#title2">Musicology</a></span>
					<span class="nav-item"><a href="areas#title3">Hermeneutic And Linguistic</a></span>
					<span class="nav-item"><a href="areas#title4">Social And Culture</a></span>
					<span class="nav-item"nav-item><a href="areas#title5">Scriptural Translations</a></span>
                  	<span class="nav-item"><a href="index">Research</a></span>
                      
					<span class="nav-item">  <a href="research">Publications</a></span>
					<span class="nav-item">   <a href="research_publications">Research Publications</a></span>
					<span class="nav-item">  <a href="completed_projects">Completed Projects</a></span>
					<span class="nav-item"><a href="underprogress">Projects Under Progress</a></span>
					<span class="nav-item"> <a href="activities">Activities</a></span>
					<span class="nav-item"><a href="activities">Manthan</a></span>
					<span class="nav-item"><a href="activities#services1">Lectures</a></span>
					<span class="nav-item"> <a href="activities#services1">Seminars</a></span>
					<span class="nav-item"> <a href="activities#services2">Councelling Services</a></span>
					<span class="nav-item" style="background-color:#362626"><a href="#">Gallery</a></span>
					<span class="nav-item"><a href="gallery_foundation">Foundation Stone</a></span>
					<span class="nav-item"> <a href="gallery_inaugration">Inauguration</a></span>
					<span class="nav-item"><a href="gallery_manthan">Manthan</a></span>
					<span class="nav-item"><a href="gallery_seminars">Seminars</a></span>
					<span class="nav-item"><a href="gallery_lectures">Special lectures</a></span>
					<span class="nav-item"><a href="gallery_workshop">Workshops</a></span>
					<span class="nav-item"><a href="gallery_fieldwork">Fieldwork</a></span>
			
	
	</div>
        <div class="col-xs-12 col-sm-12 col-lg-6">
            <div id="head_wrap">
                <div id="header">
					<span class="in-nav ion-navicon"></span>
                    <a href="index" >
                        <h2 class="toph hset colorido-t" style="font-family:'algerian' !important;text-align:center;">GYAN ANJAN</h2></a><a href="http://www.gndu.ac.in"><h4 class="toph hset colorido-t" style="font-family:'algerian' !important;text-align:center; margin-top:5.5%;"> Guru Nanak Dev University </h4></a>
                       
                </div>
            </div>
        </div>
        <div class="col-md-3"></div>
        <div class="col-lg-3 col-sm-12">
            <form class="style" action="search_results.php">
                <div class="input-group">
                    <input id="tipue_search_input" autocomplete="off" required type="text" name="q" class="form-control len" placeholder="Search for...">
                    <span class="input-group-btn">
                        <button class="btn btn-default colorido" type="submit"><i class="glyphicon glyphicon-search"></i></button>
                    </span>

                </div>
            </form>
            <!-- <div class="text-center"><a href="admin"><span class="glyphicon glyphicon-lock"></span> Login</a></div>-->
        </div>



   


    <nav class="navbar navbar-fixed-top container colorido-gd" style="margin-top:80px;z-index:800000000">
        <div class="container">
            <div class="nav-head">
                <button type="button" class="navbar-toggle collapsed" data-toggle="collapse" data-target="#bs-example-navbar-collapse-1" aria-expanded="false">
                    <a href="index" > <span class="sr-only">gyan anjan</span></a>
                    <span class="icon-bar" style="background-color:wheat"></span>
                    <span class="icon-bar" style="background-color:wheat"></span>
                    <span class="icon-bar" style="background-color:wheat"></span>
                </button>
                <br>
                <a href="index" style="color:white"> <span class="title" style="font-family:ralvey;font-size:24px; ">Centre on Studies in Sri Guru Granth Sahib</span></a>
            </div>
            <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1" style="margin-right:0;">
                <ul class="nav navbar-nav pull-right" style="">

                    <li>
                        <a href="index">Home</a>
                    </li>

                    <li class="dropdown">
                        <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">About Us<span class="caret"></span></a>
                        <ul class="dropdown-menu">
                            <li><a href="infrastructure">Infrastructure</a></li>
                            <li><a href="faculty">Faculty</a></li>
                            <li><a href="contact_us">Contact us</a></li>
                        </ul>
                    </li>

                    <li class="dropdown">
                        <a href="areas" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">Areas<span class="caret"></span></a>
                        <ul class="dropdown-menu">
                            <li><a href="areas#title">Interfaith Understanding</a></li>
                            <li><a href="areas#title1">Scriptural Studies</a></li>
                            <li><a href="areas#title2">Musicology</a></li>
                            <li><a href="areas#title3">Hermeneutic And Linguistic</a></li>
                            <li><a href="areas#title4">Social And Culture</a></li>
                            <li><a href="areas#title5">Scriptural Translations</a></li>
                        </ul>
                    </li>

                    <li class="dropdown">
                        <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">Research<span class="caret"></span></a>
                        <ul class="dropdown-menu">
                            <li><a href="research">Publications</a></li>
                            <li><a href="research_publications">Research Publications</a></li>
                            <li><a href="completed_projects">Completed Projects</a></li>
                            <li><a href="underprogress">Projects Under Progress</a></li>
                            <li><a href="playlist">Important Videos</a></li>
                        </ul>
                    </li>

                    <li class="dropdown">
                        <a href="activities" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">Activities<span class="caret"></span></a>
                        <ul class="dropdown-menu">
                            <li><a href="activities">Manthan</a></li>
                            <li><a href="activities#services1">Lectures</a></li>
                            <li><a href="activities#services1">Seminars</a></li>
                            <li><a href="activities#services2">Councelling Services</a></li>
                        </ul>
                    </li>

                    <li class="dropdown">
                        <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">Gallery<span class="caret"></span></a>
                        <ul class="dropdown-menu">
                            <li><a href="gallery_foundation">Foundation Stone</a></li>
                            <li><a href="gallery_inaugration">Inauguration</a></li>
                            <li><a href="gallery_manthan">Manthan</a></li>
                            <li><a href="gallery_seminars">Seminars</a></li>
                            <li><a href="gallery_lectures">Special lectures</a></li>
                            <li><a href="gallery_workshop">Workshops</a></li>
                            <li><a href="gallery_fieldwork">Fieldwork</a></li>
                        </ul>
                    </li>
                     <li class="dropdown">
                                <a href="blog"  role="button" aria-haspopup="true" aria-expanded="false"><span class="glyphicon glyphicon-pencil"></span>  Blogs</a>
                               
                            </li>
                         
                </ul>
            </div>
        </div>
    </nav>
</div>